...
import { getBlockExplorerTxLink, getParsedError, notification } from "@/libs/scaffold-eth/utils";
import type { TransactorFuncOptions } from "@/libs/scaffold-eth/utils/contract";
...
const TxnNotification = ({
  message,
  blockExplorerLink,
}: { message: string; blockExplorerLink?: string }) => {
  return (
...
export const useTransactor = (_walletClient?: WalletClient): TransactionFunc => {
  ...
  const result: TransactionFunc = async (tx, options) => {
    ...
    try {
      ...
      const publicClient: any = getPublicClient(reownWagmiAdapter.wagmiConfig);
      ...
      if (transactionReceipt?.status === "reverted") {
        notification.error(
          <TxnNotification message={message} blockExplorerLink={blockExplorerTxURL} />,
        );
        throw error;
      }
